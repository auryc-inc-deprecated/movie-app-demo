...
- (void)getSimilarMovies:(NSString *)movieId numberOfPages:(NSString *)numberOfPages completion:(KMSimilarMoviesCompletionBlock)completionBlock
{
    if (completionBlock)
    {
        ...
        [manager GET:[self prepareUrl:movieId] parameters:nil progress:nil success:^(NSURLSessionTask *task, id responseObject) {
            
            //NSLog(@"JSON: %@", responseObject);
            
//            NSDictionary* infosDictionary = [self dictionaryFromResponseData: responseObject jsonPatternFile:@"KMSimilarMoviesSourceJsonPattern.json"];
